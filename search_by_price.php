...
if(isset($customer_id))
{
    ...
    ?>
<h1 class="text text-capitalize text-center text-warning" style="color: Black">Customer Search Product By Price</h1>
<form method="post">
    <table align="center">
        <tr>
            <td>
                Enter Product Price From:
            </td>
            <td>
                <input type="number" name="sprice" required="" placeholder="Enter Product Price for Search">
                
            </td>
        </tr>
        <tr>
            <td>
                Enter Product Price To:
            </td>
            <td>
                <input type="number" name="eprice" required="" placeholder="Enter Product Price for Search">
...
<?php
  ...
  if(isset($_POST['done']))
  {
      $sprice=$_POST['sprice'];
      $eprice=$_POST['eprice'];
 $query="SELECT * FROM `product` where p_price>'$$sprice' AND p_price<'$eprice'";
